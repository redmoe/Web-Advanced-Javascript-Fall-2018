	...
	//	console.log((window.innerWidth || document.body.clientWidth));
		...
		player[0] = new Player(width-border.x-grid/2, 180,-1,0,1,color(0,0,255));
		player[0].Morph(player[0].form);

		player[1] = new Player(border.x+grid/2, 0,1,1,0,color(255,0,0));
	...
	function Player(tempX, ang, snakeDir,index,otherIndex,color) {
		...
		this.dir = snakeDir;
		//this.sprite.addToGroup ( spriteGroup );1
		this.form = "Pong";
		...
		this.Move = function() {
			...
			//	ball.sprite.collide(this.sprite);
				//ball.sprite.collide(this.cells);
				if (ball.sprite.bounce(this.sprite) || ball.sprite.bounce(	this.cells)) {
					noises[1].play();
				}
				
				// if (ball.sprite.bounce(this.sprite) || ball.sprite.bounce(	this.cells)) {
				// 	//ball.sprite.setSpeed(ball.speed,ball.sprite.rotation);
				// }
		...
		this.GridMove = function () {
			...
			//	console.log((180*Math.atan2(this.snakeControl.x, this.snakeControl.y)/PI)95));
				...
				//if (player[this.opIndex].sprite.overlapPoint (this.sprite.position.x,this.sprite.position.y)) {
			...
			//	if (this.sprite.overlap(player[this.opIndex].sprite)) {
				//	console.log(player[this.opIndex].sprite);
				//	console.log(this.sprite);
					console.log(this.sprite.position.x);
				//	console.log(player[this.opIndex].sprite.position.x);
					for (var i = 0; i < this.cells.length; i++) {
						this.cells[i].shapeColor = 255;
					}
					this.Reset();
					noises[3].play();
				//	player[this.opIndex].Reset();
				}
				// else if (this.cells.overlap(this.sprite)) {
				// 	this.Reset();
				// 	noises[3].play();
				// }
				// else if (player[this.opIndex].cells.overlap(this.sprite)) {
				// 	this.Reset();
				// 	noises[3].play();
				// }	


			// 	if (player[this.opIndex].sprite.overlap(this.sprite)) {
			// //	if (this.sprite.overlap(player[this.opIndex].sprite)) {
			// 	//	console.log(player[this.opIndex].sprite);
			// 	//	console.log(this.sprite);
			// 		console.log(this.sprite.position.x);
			...
			// 	}	
				...
				// if (player[this.opIndex].sprite.position.x == this.sprite.position.x && player[this.opIndex].sprite.position.y ==this.sprite.position.y) {
				// 	//this.Reset();
				// 	///player[this.opIndex].Reset();
				// 	console.log(player[this.opIndex].sprite.position.x );
				...
				// 	if (Math.floor(this.cells[c].position.x) == Math.floor(this.sprite.position.x) && Math.floor(this.cells[c].position.y) == Math.floor(this.sprite.position.y)) {
				...
				// for (var c = 0; c < player[this.opIndex].cells.length-1; c++) {
				// 	if (Math.floor(player[this.opIndex].cells[c].position.x) == Math.floor(this.sprite.position.x) && Math.floor(player[this.opIndex].cells[c].position.y) == Math.floor(this.sprite.position.y)) {
				// 	//if (this.cells[c].overlap(this.sprite)) {
				// 		this.Reset();
				// 		return;
				// 	}
		...
		this.Draw = function () {
		  ...
		  //	console.log(this.sprite);
		  	console.log(		this.sprite.shapeColor);
		    drawSprite(this.sprite);
		  	fill(0);
		  	textSize(grid);
	...
	//	rect(border.x/2,border.y*5.5,border.x,height);
		...
		//if (testObject1.mouseIsOver) {
			//console.log(testObject1.mouseIsOver);
		//}/
	}
	function CreateTesting () {
		//testObject1 = createSprite(width/2,height/2,10,10);
		//testObject1.mouseActive = true;
